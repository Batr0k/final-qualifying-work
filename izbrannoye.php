...
<div class="topnav">
  ...
  <div class="topnav-right"> <?php
    if(isset($_COOKIE["Вход"])){ ?>
      ...
      <a  class="active" href="http://localhost/diplom/izbrannoye.php">Избранное</a>
...
$account=$_COOKIE["Вход"];
           $meropriyatiya = mysqli_query( $connect, "SELECT `name`, `opisaniye`, `adres`, `path`, `meropriyatiya`.`id`
              FROM `meropriyatiya` JOIN `likes` on `meropriyatiya`.`id`=`likes`.`idMeropriyatiya` join `account` on `likes`.`idAccount`=`account`.`id`
