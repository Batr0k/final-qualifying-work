...
<div class="topnav">
  ...
  <a  class="active" href="http://localhost/diplom/kino.php">Кино</a>
...
<?php
           $meropriyatiya = mysqli_query( $connect, "SELECT `meropriyatiya`.`name`, `opisaniye`, `adres` , `path` FROM `meropriyatiya`
             JOIN  `tipymeropriyatiy` ON `tipymeropriyatiy`.`id`=`meropriyatiya`.`idtipa` WHERE `tipymeropriyatiy`.`id`=1");
